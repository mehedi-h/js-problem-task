...
function nasta (singaraQuantity, somucaQuantity, jilapiQuantity) {
    const singaraPrice = 7;
    const somucaPrice = 10;
    const jilapiPrice = 15;

        const totalSingaraPrice = (singaraQuantity * singaraPrice);
        const totalSomucaPrice = (somucaQuantity * somucaPrice);
        const totalJilapiPrice = (jilapiQuantity * jilapiPrice);

        const totalPrice = totalSingaraPrice + totalSomucaPrice + totalJilapiPrice;
    return totalPrice;
}

const totalPrice = nasta(5,3,2);
console.log('Mama Singara, Somuca O Jilapir bill', totalPrice,'taka den.');